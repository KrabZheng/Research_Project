...
## ---- Paths ------------------------------------------------------------------
raw_path      <- "IC50/Resazurin_Exp_34/20250804_Resazurin ASC397 ASC397cse ASC399 ASC399cse Rotenone H2O2.txt"
compound_path <- "IC50/Resazurin_Exp_34/Resazurin_Exp_34_Compound.txt"
sample_path   <- "IC50/Resazurin_Exp_34/Resazurin_Exp_34_Sample.txt"
...
resample_fit_once <- function(df_repl){
  ...
  df_trim <- df_repl %>%
    ...
    mutate(med = median(Viability, na.rm=TRUE),
           ...
           mad_mult = ifelse(n() <= 3, 4, 3),  # Less aggressive for small samples
           keep = ifelse(
             is.finite(madv) & madv > 0,
             abs(Viability - med) <= mad_mult * madv,
             TRUE
           )
         ) %>%
    ungroup() %>% 
    filter(keep)  df_ag <- df_trim %>% group_by(Dose) %>%
...
if (length(rows) == 0) stop("No '_Fluo' blocks found.")reps_per_grp <- 12 / length(group_names)
...
methods_blurb <- function(cfg){
  ...
  paste(
    ...
    "was chosen by AICc, preferring the simpler model when ΔAICc ≤ 2. Fits required at least ", N, " unique positive doses and used increased iteration limits (drc::drm).",
    "",
...
print(plot_across(all_preds_ci, all_viability, "B1"))
...
## ---- Export optional ---------------------------------------------------------
# write_csv(all_ic50, "Resazurin_Exp34_IC50_results-2.csv")
