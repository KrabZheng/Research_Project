...
compound_path <- "Resazurin_IC50/Resazurin_Exp_35_Compound.txt"
  ...
  viab <- map_dfr(rows, function(i){
    ...
    sig %>%
      ...
      mutate(
        ...
        Patient    = str_remove(Group, "(_)?cse$"),
        Condition  = ifelse(str_detect(Group, "(_)?cse$"), "CSE", "Control"),        Condition  = ifelse(str_detect(Group, "cse$|_cse$"), "CSE", "Control"),
        CompoundID = PlateID
      ) %>%
...
"was chosen by AICc, preferring the simpler model when ΔAICc ≤ 2. Fits required at ",
"least ", N, " unique positive doses and used increased iteration limits (drc::drm).","Uncertainty in the mean curve was quantified via parametric bootstrap draws ",
...
## ---- Export optional ---------------------------------------------------------
# write_csv(all_ic50, "Resazurin_Exp35_IC50_results.csv")
