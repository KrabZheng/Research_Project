...
channel_mode     <- "Fluo"           # "Fluo" or "AbsMinusBG" (not used in this parser)
...
# Balanced column→slot mapper (safe when 12 %% n_slots != 0)
slot_for_col <- function(col, n_slots){
  n_slots <- pmax(1L, as.integer(n_slots))
  base <- rep(seq_len(n_slots), length.out = 12L)
  base[col]
}
...
get_viability_one_plate <- function(raw_path){
  ...
  one_block <- function(i){
    ...
    if (!is.na(m1[1,1])) {
      sample_id  <- m1[1,2]
      set_letter <- m1[1,3]
      slot_from_header <- NA_integer_
    } else if (!is.na(m2[1,1])) {
      sample_id  <- NA_character_
      set_letter <- m2[1,2]
      slot_from_header <- as.integer(m2[1,3])
    } else {
    ...
    n_slots_this <- dplyr::coalesce(slots_info$n_slots[match(set_letter, slots_info$SetLetter)], 1L)
    ...
    df <- sig %>%
      ...
      dplyr::mutate(
        ...
        SlotIndex = ifelse(!is.na(slot_from_header),
                           slot_from_header,
                           slot_for_col(Col, n_slots_this)),
    ...
    if (have_sample_map) {
      df <- df %>%
        dplyr::mutate(
          ColGroup = ceiling(Col / (12 / length(group_names))),
          GroupRaw = group_names[pmax(1, pmin(ColGroup, length(group_names)))]
        )
    } else {
      df <- df %>% dplyr::mutate(GroupRaw = ifelse(is.na(sample_id), paste0("Set", SetLetter), sample_id))
    }

    df %>%
      dplyr::mutate(
        Group    = ifelse(stringr::str_detect(GroupRaw, "(?i)(_)?cse$"),
                          paste0(stringr::str_remove(GroupRaw, "(?i)(_)?cse$"), "cse"),
                          GroupRaw),
        Patient  = stringr::str_remove(Group, "(?i)(_)?cse$"),
        Condition= ifelse(stringr::str_detect(Group, "(?i)cse$"), "CSE", "Control"),
  ...
  viab <- viab %>%
    dplyr::group_by(Experiment, Group, CompoundName) %>%
    dplyr::mutate(
...
all_viability <- purrr::map_dfr(raw_files, get_viability_one_plate) %>%
  ...
  dplyr::mutate(
    ...
    # *** critical separation ***: per-experiment keys
    CmpdKey    = paste(CompoundName, Experiment, sep = "||"),
    GroupKey   = paste(CompGroup,    Experiment, sep = "||")
  )
...
bootstrap_mean_matrix <- function(fit, df, grid, B, b_bounds = cfg$b_draw_bounds, e_bounds = NULL){
  ...
  # fall back to nonparametric replicate bootstrap
  Bnp <- min(B, 500)
...
predict_curves <- function(viab, n = 200, level = 0.95){
  ...
  viab %>%
    dplyr::group_by(Experiment, Sample, Patient, CompoundName, UnitStd) %>%  # << include Experiment
    tidyr::nest() %>%
    dplyr::mutate(
...
ensure_dir(out_dir)
readr::write_csv(
  all_ic50 %>% dplyr::arrange(Experiment, CompoundName, Sample, Patient),
  file.path(out_dir, "IC50_results.csv")
)
readr::write_csv(
  qc_df %>% dplyr::arrange(Experiment, CompoundName, Sample, Patient),
...
plot_single <- function(experiment, sample, compound){
  dline <- dplyr::filter(all_preds_ci, Experiment == experiment, Sample == sample, CompoundName == compound)
  dpts  <- dplyr::filter(all_viability, Experiment == experiment, Sample == sample, CompoundName == compound,
                         Dose > 0, is.finite(Dose))
  if (!nrow(dpts) || !nrow(dline)) return(NULL)
  ...
  ggplot() +
    ...
    labs(title = glue("{compound} — {sample} ({experiment})"), x = xlab, y = "Viability (%)") +
    base_theme
}

# across samples: base compound + co-treatments, within one experiment
plot_compound_across_samples <- function(experiment, comp_group){
  d <- all_viability %>% dplyr::filter(Experiment == experiment, CompGroup == comp_group, Dose > 0)
  if (!nrow(d)) return(NULL)
  cmp_list <- unique(d$CompoundName)
  dline <- all_preds_ci %>% dplyr::filter(Experiment == experiment, CompoundName %in% cmp_list)
  if (!nrow(dline)) return(NULL)
  xlab  <- unit_label_for(dline$UnitStd)
  ...
  ggplot() +
    ...
    labs(title = glue("{comp_group} and co-treatments — across samples ({experiment})"),
         x = xlab, y = "Viability (%)", colour = "Treatment") +
    base_theme + theme(legend.position = "bottom")
}

# within a sample: base vs co-treatments, within one experiment
plot_sample_vs_cotreats <- function(experiment, sample, comp_group){
  d <- all_viability %>% dplyr::filter(Experiment == experiment, Sample == sample, CompGroup == comp_group, Dose > 0)
  if (!nrow(d)) return(NULL)
  cmp_list <- unique(d$CompoundName)
  dline <- all_preds_ci %>% dplyr::filter(Experiment == experiment, Sample == sample, CompoundName %in% cmp_list)
  if (!nrow(dline)) return(NULL)
  xlab  <- unit_label_for(dline$UnitStd)
  ...
  ggplot() +
    ...
    scale_x_log10(labels = label_number()) +
    coord_cartesian(ylim = c(0, 100)) +
    labs(title = glue("{comp_group} — {sample} ({experiment}) base vs co-treatments"),
         x = xlab, y = "Viability (%)", colour = "Treatment") +
    base_theme + theme(legend.position = "bottom")
}

## ---- Save plots --------------------------------------------------------------
# singles (per experiment)
all_viability %>% dplyr::distinct(Experiment, Sample, CompoundName) %>%
  purrr::pwalk(function(Experiment, Sample, CompoundName){
    gp <- plot_single(Experiment, Sample, CompoundName)
    save_plot(gp, file.path(plots_dir, glue("{CompoundName}__{Sample}__{Experiment}")))
  })

# across-sample combined (per experiment × base group)
all_viability %>% dplyr::distinct(Experiment, CompGroup) %>%
  purrr::pwalk(function(Experiment, CompGroup){
    gp <- plot_compound_across_samples(Experiment, CompGroup)
    save_plot(gp, file.path(plots_comb_dir, glue("{CompGroup}__across_samples__{Experiment}")))
  })

# within-sample combined (per experiment × sample × base group)
all_viability %>% dplyr::distinct(Experiment, Sample, CompGroup) %>%
  purrr::pwalk(function(Experiment, Sample, CompGroup){
    gp <- plot_sample_vs_cotreats(Experiment, Sample, CompGroup)
    if (!is.null(gp)) save_plot(gp, file.path(plots_comb_dir, glue("{CompGroup}__{Sample}__{Experiment}")))
  })
